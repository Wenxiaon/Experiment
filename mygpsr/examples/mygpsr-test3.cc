...
bool
MyGpsrExample::Configure (int argc, char **argv)
{
  // Enable MYGPSR logs by default. Comment this if too noisy
  //LogComponentEnable("MyGpsrRoutingProtocol", LOG_LEVEL_ALL);
...
//  Config::SetDefault ("ns3::WifiRemoteStationManager::RtsCtsThreshold", UintegerValue (1)); // enable rts cts all the time.
  ...
  MyGpsrHelper mygpsr;
  mygpsr.Set ("LocationServiceName", StringValue ("GOD"));
  mygpsr.Install ();
...
void
MyGpsrExample::Report (std::ostream &)
{
}

void
MyGpsrExample::CreateNodes ()
{
...
MyGpsrExample::CreateDevices ()
{
  ...
  if (pcap)
    {
      wifiPhy.EnablePcapAll (std::string ("mygpsr"));
    }
}

void
MyGpsrExample::InstallInternetStack ()
{
  MyGpsrHelper mygpsr;
  // you can configure MYGPSR attributes here using mygpsr.Set(name, value)
  InternetStackHelper stack;
  stack.SetRoutingHelper (mygpsr);
  stack.Install (nodes);
...
void
MyGpsrExample::InstallApplications ()
{
